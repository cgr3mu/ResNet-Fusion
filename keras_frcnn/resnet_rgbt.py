...
from keras.layers import Input, Add, Dense, Activation, Flatten, Convolution2D, MaxPooling2D, ZeroPadding2D, \
    AveragePooling2D, TimeDistributed, add
...
def nn_base(input_tensor_1 = None,input_tensor_2 = None, trainable=False):
    ...
    with tf.device('/gpu:0'):
	    ...
	    x = Convolution2D(64, (7, 7), strides=(2, 2),kernel_initializer = 'he_normal' , name='rgbconv1', trainable = trainable)(x)
	    x = FixedBatchNormalization(axis=bn_axis, name='bn_rgbconv1')(x)
	    x = Activation('relu')(x)
	    
	    x = Convolution2D(64, (3, 3), padding = 'same', kernel_initializer = 'he_normal' ,name='rgbconv2', trainable = trainable)(x)
	    x = FixedBatchNormalization(axis=bn_axis, name='bn_rgbconv2')(x)
	    x = Activation('relu')(x)
	    x = MaxPooling2D((3, 3), strides=(2, 2))(x)
	    
	    y = ZeroPadding2D((3, 3), name = 'therm_zero_padding')(thermal_input)
	    y = Convolution2D(64, (7, 7), strides=(2, 2), kernel_initializer = 'he_normal' ,name='thermconv1', trainable = trainable)(y)
	    y = FixedBatchNormalization(axis=bn_axis, name='bn_thermconv1')(y)
	    y = Activation('relu')(y)
	    y = Convolution2D(64, (3, 3), padding = 'same',kernel_initializer = 'he_normal' ,name='thermconv2', trainable = trainable)(y)
	    ...
	    x = conv_block(z, 3, [64, 64, 256], stage=2, block='a', strides=(1, 1), trainable = trainable)

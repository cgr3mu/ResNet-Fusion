	...
	while True:
		...
		for img_data in all_img_data:
			try:
				...
				x2_img[:, :, 0] -= C.img_channel_mean[0]
				x2_img[:, :, 1] -= C.img_channel_mean[1]
				x2_img[:, :, 2] -= C.img_channel_mean[2]
				x2_img /= C.img_scaling_factor

				x2_img = np.transpose(x2_img, (2, 0, 1))
				x2_img = np.expand_dims(x2_img, axis=0)

				y_rpn_regr[:, y_rpn_regr.shape[1]//2:, :, :] *= C.std_scaling

				if backend == 'tf':
					x_img = np.transpose(x_img, (0, 2, 3, 1))
					x2_img = np.transpose(x2_img, (0, 2, 3, 1))
					y_rpn_cls = np.transpose(y_rpn_cls, (0, 2, 3, 1))
					y_rpn_regr = np.transpose(y_rpn_regr, (0, 2, 3, 1))

				yield np.copy(x_img), np.copy(x2_img), [np.copy(y_rpn_cls), np.copy(y_rpn_regr)], img_data_aug
